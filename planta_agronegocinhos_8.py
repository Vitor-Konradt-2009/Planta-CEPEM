...
# Admins solicitados
ADMIN_SEEDS = {
    "nelise.ruscheinsky@escola.pr.gov.br": {
        "nome": "Nelise Ruscheinsky",
        "senha": "agrocepem",
    },
    "alexandra.martinez@escola.pr.gov.br": {
        "nome": "Alexandra da Silva Martinez",
        "senha": "agrocepem",
    },
    "victor.luiz.marchi@escola.pr.gov.br": {
        "nome": "Victor Luiz Marchi",
        "senha": "agrocepem",
    },
    "s.claudinei@escola.pr.gov.br": {
        "nome": "Claudinei Ferreira da Silva",
        "senha": "agrocepem",
    },
}
...
<head>
  ...
  <style>
    * { box-sizing: border-box; font-family: Arial, sans-serif; }
    ...
    .left-nav { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
    .right-info { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
    ...
    .badge {
      display:inline-block; font-size:12px; font-weight:bold; border-radius:999px; padding:4px 8px;
    }
    ...
    .post {
      border: 1px solid #e5e7eb; border-radius: 10px; padding: 12px; background: #fff; margin-bottom: 10px;
    }
...
def init_db():
    ...
    cols_admin = [c[1] for c in cur.fetchall()]
    if "nome" not in cols_admin:
        cur.execute("ALTER TABLE admin_users ADD COLUMN nome TEXT NOT NULL DEFAULT ''")
    if "ativo" not in cols_admin:
        cur.execute("ALTER TABLE admin_users ADD COLUMN ativo INTEGER NOT NULL DEFAULT 1")
    if "criado_em" not in cols_admin:
        cur.execute("ALTER TABLE admin_users ADD COLUMN criado_em TEXT NOT NULL DEFAULT ''")

    cur.execute("PRAGMA table_info(inscricoes)")
    cols_insc = [c[1] for c in cur.fetchall()]
    if "acesso_aluno_ativo" not in cols_insc:
        cur.execute("ALTER TABLE inscricoes ADD COLUMN acesso_aluno_ativo INTEGER NOT NULL DEFAULT 0")
    if "motivo_negacao" not in cols_insc:
        cur.execute("ALTER TABLE inscricoes ADD COLUMN motivo_negacao TEXT")
    if "decidido_por" not in cols_insc:
        cur.execute("ALTER TABLE inscricoes ADD COLUMN decidido_por TEXT")
    if "data_decisao" not in cols_insc:
        cur.execute("ALTER TABLE inscricoes ADD COLUMN data_decisao TEXT")
    if "status" not in cols_insc:
        cur.execute("ALTER TABLE inscricoes ADD COLUMN status TEXT NOT NULL DEFAULT 'Pendente'")

    cur.execute("PRAGMA table_info(alunos_users)")
    cols_aluno = [c[1] for c in cur.fetchall()]
    if "ativo" not in cols_aluno:
        cur.execute("ALTER TABLE alunos_users ADD COLUMN ativo INTEGER NOT NULL DEFAULT 1")
...
def inscricao():
    if request.method == "POST":
        ...
        """, (
            nome, data_nascimento, cpf_num, email, telefone, turma, 1,
            datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        ))
        conn.commit()
        conn.close()
    ...
    <section class="card">
      ...
      <p>
        ...
        {% if resultado['status'] == 'Aceita' %}
          ...
          {% if resultado['acesso_aluno_ativo'] == 1 %}
...
def admin_info():
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT nome, email, criado_em FROM admin_users WHERE email = ?", (session.get("admin_email"),))
    admin = cur.fetchone()
    conn.close()
...
def admin_config():
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM admin_users WHERE email = ?", (session.get("admin_email"),))
    admin = cur.fetchone()
...
def admin_inscritos():
    ...
    cur.execute("""
        ...
        WHERE status = 'Pendente'
           OR (status = 'Aceita' AND acesso_aluno_ativo = 1)
        ORDER BY id DESC
    """)
    rows = cur.fetchall()
    conn.close()
...
def admin_postagens():
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT id, conteudo, autor_email, criado_em
        FROM postagens
        WHERE ativo = 1
        ORDER BY id DESC
    """)
    ...
    <section class="card">
      ...
      {% for p in postagens %}
        <div class="post">
          <small><strong>{{ p['autor_email'] }}</strong> • {{ p['criado_em'] }}</small>
          <p>{{ p['conteudo'] }}</p>
        </div>
      {% else %}
...
def negar_inscricao(inscricao_id):
    ...
    # Se já houver conta, remove acesso
    cur.execute("""
        UPDATE alunos_users
...
def aluno_primeiro_acesso():
    if request.method == "POST":
        ...
        """, (
            ...
            generate_password_hash(senha), datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        ))
        conn.commit()
        conn.close()
...
def aluno_dashboard():
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT conteudo, autor_email, criado_em
        FROM postagens
        WHERE ativo = 1
        ORDER BY id DESC
    """)
