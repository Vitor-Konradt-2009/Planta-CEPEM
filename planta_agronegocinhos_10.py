...
<head>
  ...
  <style>
    * { box-sizing: border-box; font-family: Arial, sans-serif; }
    ...
    .badge.espera { background:#e0e7ff; color:#3730a3; }
...
def init_db():
    ...
    if "posicao_espera" not in cols_insc:
        cur.execute("ALTER TABLE inscricoes ADD COLUMN posicao_espera INTEGER")

    cur.execute("PRAGMA table_info(admin_users)")
    cols_admin = [c["name"] for c in cur.fetchall()]
    ...
    cols_aluno = [c["name"] for c in cur.fetchall()]
...
def inscricao():
    if request.method == "POST":
        ...
        """, (nome, data_nascimento, cpf_num, email, telefone, turma, 1, now_str()))
        conn.commit()
        conn.close()
...
def admin_inscritos():
    ...
    cur.execute("""
        SELECT *
        FROM inscricoes
        WHERE status IN ('Pendente', 'Lista de Espera')
